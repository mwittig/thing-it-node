module.exports = {
    ...
    bind: function (node, device) {
        ...
        if (!node.plugins[device.plugin]) {
            ...
            if (node[device.id]) {
                throw 'Device [' + device.id + '] is already registered.';
            }

            node[device.id] = device;

            var id = node.__nodeManager.unmangleId(device.id);

            node.__executionContext[id] = device;
...
function Device() {
    ...
    Device.prototype.startDevice = function () {
        ...
        this.state = {}; //TODO Should be initialized on device-plugin level.
     ...
     *
     */
    Device.prototype.findSensor = function (id) {
        for (var n = 0; n < this.sensors.length; ++n) {
        ...
        throw "No Sensor exists with ID " + id + ".";
    };
     ...
     *
     */
    Device.prototype.publishEvent = function (event, data, relatedId) {
        for (var n = 0; n < this.eventProcessors.length; ++n) {
            this.eventProcessors[n].notifyOnDeviceEvent(this, event, data);
        }

        this.node.publishDeviceEvent(this, event, data, relatedId);
    };
    ...
    Device.prototype.publishOperationalStateChange = function () {
        for (var n = 0; n < this.eventProcessors.length; ++n) {
            this.eventProcessors[n].notifyOnDeviceOperationalStateChange(this, this.operationalState);
        }
    ...
    Device.prototype.publishStateChange = function () {
        for (var n = 0; n < this.eventProcessors.length; ++n) {
            this.eventProcessors[n].notifyOnDeviceStateChange(this, this.getState());
        }

        this.node.publishDeviceStateChange(this, this.getState());
    };

    /**
     *
     */
    Device.prototype.publishStateChangeHistory = function (history) {
        this.node.publishDeviceStateChangeHistory(this, history);
    };
     ...
     *
     */
    Device.prototype.operationalStateTestHook = function () {
        // Magic sequence to simulate non-reachable state

        if (this.lastOperationalStateTestHookCall) {
            if (new Date().getTime() - this.lastOperationalStateTestHookCall < 2000) {
                this.operationalStateTestHookCallCount++;
            }
        } else {
            this.operationalStateTestHookCallCount = 0;
        }

        this.lastOperationalStateTestHookCall = new Date().getTime();

        if (this.operationalStateTestHookCallCount > 5) {
            if (this.state.operationalState == 'notReachable') {
                this.state.operationalState = 'normal';
            } else {
                this.state.operationalState = 'notReachable';
            }
