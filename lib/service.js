...
function Service() {
     ...
     *
     */
    Service.prototype.startFlow = function () {
        if (this.node.options.proxy === "local") {
            this.logError("Cannot start Flow [" + this.content.flow + "] in local mode.");
        }
        else {
            this.logDebug("Starting Flow [" + this.content.flow + "].");
            this.node.publishFlowStart(this);
        }
    };
