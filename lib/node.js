...
function NodeManager() {
     ...
     */
    NodeManager.prototype.getNodeConfiguration = function (uuid) {
        console.log("Obtain for >>> " + uuid);
        console.log("Obtain for ||| " + (this.configurationsMap[uuid] != null));

        return this.configurationsMap[uuid];
    };
    ...
    NodeManager.prototype.initializeRestServices = function () {
        ...
        this.app.post("/configuration/devices", function (request, response) {
            ...
            this.node.createDevice(request.body, this.defaultNodeConfiguration.uuid).then(function () {
        ...
        this.app.put("/configuration/devices/:device", function (request, response) {
            this.node.updateDevice(request.params.device, request.body, this.defaultNodeConfiguration.uuid).then(function () {
                response.send(this.node.clientCopy());
            }.bind(this)).fail(function (error) {
                this.logError(error);
                response.status(500).send(error);
        ...
        this.app.delete("/configuration/devices/:device", function (request, response) {
            this.node.deleteDevice(request.params.device, this.defaultNodeConfiguration.uuid).then(function () {
                response.send(this.node.clientCopy());
            }.bind(this)).fail(function (error) {
                this.logError(error);
                response.status(500).send(error);
                                        ...
                                        if ((details = this.matchRequestPath(request.path, "/firmware")) && request.method === "POST") {
                                            ...
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                ...
                                                status: 0, data: {
                                                    ...
                                                    configuration: this.configurationCopy(request.nodeUuid)
                                                }
                                            });
                                            ...
                                            this.node.createDevice(request.body, request.nodeUuid).then(function () {
                                                this.reverseProxyNamespace.emit("httpResponse", {
                                                    uuid: request.uuid,
                                                    status: 0,
                                                    data: this.configurationCopy(request.nodeUuid)
                                                });
                                            }.bind(this)).fail(function (error) {
                                                this.logError(error);
...
function Node() {
     ...
     * @returns {*|promise}
     */
    Node.prototype.createDevice = function (newDevice, nodeUuid) {
        var deferred = q.defer();

        newDevice.id = utils.getNextDefaultId(this.devices, this.__nodeManager.mangleId(nodeUuid, "device"), 1);

        var clone = utils.cloneFiltered(newDevice);

        this.devices.push(clone);
        device.bind(this, clone).startDevice().then(function () {
            this.publishNodeConfigurationChange(nodeUuid);
     ...
     * @returns {*|promise}
     */
    Node.prototype.updateDevice = function (id, newDevice, nodeUuid) {
        var deferred = q.defer();
     ...
     * @returns {*|promise}
     */
    Node.prototype.deleteDevice = function (id, nodeUuid) {
    ...
    Node.prototype.publishNodeConfigurationChange = function (nodeConfigurationUuid) {
        ...
        try {
            if (this.namespace) {
                this.namespace.emit("nodeConfigurationChange", this.__nodeManager.configurationCopy(nodeConfigurationUuid));
            }

            if (this.reverseProxyNamespace) {
                this.reverseProxyNamespace.emit("nodeConfigurationChange", this.__nodeManager.configurationCopy(nodeConfigurationUuid));
                this.logDebug("Emitted Node Configuration Change for Node " + nodeConfigurationUuid);
            }
        }
        catch (error) {
