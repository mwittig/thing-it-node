...
function Node() {
    ...
    Node.prototype.initialize = function (options, app, io, server) {
        ...
        app.post("/login", function (req, res) {
            if (self.options.authentication.type == "none") {
                res.send({});
            }
            else if (self.options.authentication.type == "password") {
                console.log("Request");
                console.log(req.body);

                if (req.body.password == self.options.authentication.password) {
                    res.send({});
                }
                else {
    ...
    Node.prototype.initializeReverseProxy = function () {
        ...
        var reverseProxyNamespace = this.options.reverseProxy + "/reverse-proxy/" + this.uuid;
        var connectInterval = setInterval(function () {

            console.log("(Re)try initial NS connection for " + reverseProxyNamespace);

            // SocketIO reconnect semantics seem not to work as we need them

            var socket = io.connect(reverseProxyNamespace, {
                //reconnection: true,
            ...
            socket.on("connect", function () {
                console.log("Reverse Proxy connected.");

                clearInterval(connectInterval);
            });
            socket.on("disconnect", function (socket) {
                console.log("Reverse Proxy disconnected");
            ...
            socket.on("request", function (request) {
                ...
                var proxyRequest = http.request(options, function (response) {
                    response.on('data', function (data) {
                        socket.emit("response-data", {status: 0, data: data});
                    });
                    response.on('end', function () {
                        socket.emit("response-end", {status: 0});
                    });
                    response.on('error', function (error) {
                        socket.emit("response-error", {status: 500, error: error});
                    });
                });
     ...
     *
     */
    Node.prototype.getEventNamespaceUrl = function () {
        if (this.options.reverseProxy) {
            this.options.reverseProxy + "/reverse-proxy/nodes/" + this.uuid + "/server/events"
        }
        else {
            this.namespacePrefix + "/events"
        }
    };
    ...
    Node.prototype.start = function (app, io) {
        ...
        this.namespace = this.io.of(this.getEventNamespaceUrl());
