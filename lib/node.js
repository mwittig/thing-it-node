...
function Node() {
    ...
    Node.prototype.initializeReverseProxy = function () {
        var socket = io.connect(this.options.reverseProxy + "/reverse-proxy/4711/");
        var interval;
        ...
        socket.on("http-proxy-request", function (request) {
            ...
            socket.emit("http-proxy-response", {body: "What a body"});
            ...
            if (!interval) {
                ...
                interval = setInterval(function () {
                    ...
                    socket.emit("http-proxy-emit", {name: "message", content: "Message " + new Date()});
                }, 5000);
            }
        });
    }
