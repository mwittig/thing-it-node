...
var REQ_NODE_FIRMWARE = '/nodes/:node/firmware';
var GET = 'GET';
var POST = 'POST';
var PUT = 'PUT';
var DELETE = 'DELETE';

var REQ_NODE_PLUGINS = '/nodes/:node/plugins';
var REQ_NODE_STATE = '/nodes/:node/state';
var REQ_NODE_CONFIG = '/nodes/:node/configuration';
var REQ_NODE_CONFIG_UNMANGLED = '/nodes/:node/configurationUnmangled';
var REQ_NODE_POLL = '/nodes/:node/poll';
var REQ_NODE_CONFIGURE = '/nodes/:node/configure';
var REQ_NODE_START = '/nodes/:node/start';
var REQ_NODE_STOP = '/nodes/:node/stop';
var REQ_NODE_RESTART = '/nodes/:node/restart';
var REQ_NODE_DEVICE_STREAM = '/nodes/:node/devices/:device/streams/:stream';
var REQ_NODE_DEVICE_REGISTER = '/nodes/:node/devices/:device/register';
var REQ_NODE_DEVICE_DISCARD = '/nodes/:node/devices/:device/discard';
var REQ_NODE_SERVICE = '/nodes/:node/services/:service';
var REQ_NODE_DEVICE_SERVICE = '/nodes/:node/devices/:device/services/:service';
var REQ_NODE_ACTOR_SERVICE = '/nodes/:node/devices/:device/actors/:actor/services/:service';
var REQ_NODE_SENSOR_SERVICE = '/nodes/:node/devices/:device/sensors/:sensor/services/:service';
var REQ_NODE_SENSOR_STATE = '/nodes/:node/devices/:device/sensors/:sensor/state/:state';
var REQ_NODE_STORYBOARD = '/nodes/:node/storyboards/:storyboard/:operation';
var REQ_NODE_DATA = '/nodes/:node/data/:data';
var REQ_NODE_PUSHALLSTATES = '/nodes/:node/pushAllStates';
var REQ_NODE_CONFIG_DEVICES = '/nodes/:node/configuration/devices';
var REQ_NODE_CONFIG_DEVICE = '/nodes/:node/configuration/devices/:device';
var REQ_NODE_CONFIG_ACTORS = '/nodes/:node/configuration/devices/:device/actors';
var REQ_NODE_CONFIG_ACTOR = '/nodes/:node/configuration/devices/:device/actors/:actor';
var REQ_NODE_CONFIG_SENSORS = '/nodes/:node/configuration/devices/:device/sensors';
var REQ_NODE_CONFIG_SENSOR = '/nodes/:node/configuration/devices/:device/sensors/:sensor';
var REQ_NODE_CONFIG_SERVICES = '/nodes/:node/configuration/services';
var REQ_NODE_CONFIG_SERVICE = '/nodes/:node/configuration/services/:service';
var REQ_NODE_CONFIG_GROUPS = '/nodes/:node/configuration/groups';
var REQ_NODE_CONFIG_GROUP = '/nodes/:node/configuration/groups/:group';
var REQ_NODE_CONFIG_EPS = '/nodes/:node/configuration/eventProcessors';
var REQ_NODE_CONFIG_EP = '/nodes/:node/configuration/eventProcessors/:eventProcessor';
var REQ_NODE_CONFIG_STORYBOARDS = '/nodes/:node/configuration/storyboards';
var REQ_NODE_CONFIG_STORYBOARD = '/nodes/:node/configuration/storyboards/:storyboard';
var REQ_NODE_CONFIG_JOBS = '/nodes/:node/configuration/jobs';
var REQ_NODE_CONFIG_JOB = '/nodes/:node/configuration/jobs/:job';
var REQ_NODE_FILES = '/nodes/:node/files';
var REQ_NODE_LOGS = '/nodes/:node/logs';
var REQ_NODE_RESOURCE_STATE = '/nodes/:node/devices/:device/state/:state/resource';
var REQ_NODE_UPDATE_ENTITLEMENTS = '/nodes/:node/devices/:device/updateEntitlements';
...
function NodeManager() {
    ...
    NodeManager.prototype.bootstrapNode = function (node, options, app, io, namespace, additionalPluginDirectories) {
        ...
        } else if (this.options.noBtleConfiguration) {
            this.logInfo("TIN was configured before --> Skipping Configuration Manager ");
        }
    ...
    NodeManager.prototype.unmangleId = function (id) {
        ...
        // Only do this if necessary. It isn't always for simulation
        if (id.indexOf("ti__") > -1) {
            id = id.substring(id.indexOf("ti__") + 4);
    ...
    NodeManager.prototype.cleanConfigurationCopy = function (uuid) {
        if (this.configurationsMap[uuid]) {
            return this.configurationsMap[uuid].configuration;
        } else {
            console.log('Configuration for ' + uuid + ' not found.');
        }
    };
    ...
    NodeManager.prototype.initializeRestServices = function () {
        ...
        this.app.get("/devices/:device/state", function (req, res) {
        ...
        this.app.get("/devices/:device/state/:state", function (req, res) {
            ...
            try {
                ...
                res.send(device.state[req.params.state]);
        ...
        this.app.get("/devices/:device/actors/:actor/state", function (req, res) {
            ...
            try {
                ...
                var actor = device[req.params.actor];
                if (!actor) {
                    throw new Error("Cannot find actor " + req.params.actor);
                }

                res.send(actor.state);
        ...
        this.app.get("/devices/:device/actors/:actor/state/:state", function (req, res) {
            ...
            try {
                ...
                res.send(actor.state[req.params.state]);
        ...
        this.app.get("/devices/:device/sensors/:sensor/state", function (req, res) {
            ...
            try {
                ...
                var sensor = device[req.params.sensor];
                if (!sensor) {
                    throw new Error("Cannot find sensor " + req.params.sensor);
                }
        ...
        this.app.get("/devices/:device/actors/:actor/state/:state", function (req, res) {
            ...
            try {
                ...
                res.send(sensor.state[req.params.state]);
     ...
     * @returns {*|promise}
     */
    NodeManager.prototype.updateFirmwareAndShutdown = function (version, configuration) {
        ...
        if (docker.container()) {
            ...
            docker.download(this.options.watchdog, version).then(function () {
                docker.getImage(version).then(function (image) {
                    if (image) {
                        docker.update(this.options.watchdog, version, configuration);
                    }
                }.bind(this))
            }.bind(this));
        ...
        this.connectInterval = setInterval(function () {
            ...
            getmac.getMac(function (error, macAddress) {
                ...
                request.post({
                    ...
                    this.logDebug('Response: ' + JSON.stringify(response));
                    this.logDebug('Error: ' + JSON.stringify(error));
                    this.logDebug('Body: ' + JSON.stringify(body));
                            ...
                            this.reverseProxyNamespace.on("connect", function () {
                                ...
                                this.reverseProxyNamespace.on("httpRequest", function (request) {
                                    ...
                                    try {
                                        ...
                                        if ((details = this.matchRequestPath(request, REQ_NODE_FIRMWARE, POST))) {

                                            var configuration = request.body.configuration;
                                            var version = request.body.currentFirmwareVersion;

                                            this.updateFirmwareAndShutdown(version, configuration).then(function () {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_PLUGINS, GET))) {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_STATE, GET))) {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG, GET))) {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0, data: {
                                                    state: this.state,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_UNMANGLED, GET))) {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0, data: {
                                                    state: this.state,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_POLL))) {
                                            this.node.poll();
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIGURE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_START))) {
                                            this.start().then(function () {
                                                this.reverseProxyNamespace.emit("httpResponse", {
                                                    uuid: request.uuid,
                                                    status: 0
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_STOP))) {
                                            this.stop().then(function () {
                                                this.reverseProxyNamespace.emit("httpResponse", {
                                                    uuid: request.uuid,
                                                    status: 0
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_RESTART))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_DEVICE_STREAM))) {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0
                                            });
                                        }
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_DEVICE_REGISTER))) {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0
                                            });
                                        }
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_DEVICE_DISCARD))) {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0
                                            });
                                        }
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_SERVICE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_DEVICE_SERVICE))) {

                                            this.logInfo('XXXXXXX: REQ_NODE_DEVICE_SERVICE: '
                                                + this.node[this.mangleId(details.params.node,
                                                    details.params.device)][details.params.service]);

                                            if (!this.node[this.mangleId(details.params.node, details.params.device)]
                                                    [details.params.service]) {
                                                throw "Service [" + details.params.service + "] does not exist";
                                            }
                                            else {
                                                ...
                                                try {
                                                    this.node[this.mangleId(details.params.node, details.params.device)]
                                                        [details.params.service](parameters);
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_ACTOR_SERVICE))) {
                                            if (!this.node[this.mangleId(details.params.node, details.params.device)]
                                                    [details.params.actor][details.params.service]) {
                                                throw "Service [" + this.node[this.mangleId(details.params.node, details.params.device)]
                                                    [details.params.actor][details.params.service] + "] does not exist";
                                            }
                                            else {
                                                ...
                                                try {
                                                    ...
                                                    this.node[this.mangleId(details.params.node, details.params.device)]
                                                        [details.params.actor][details.params.service](parameters);
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_SENSOR_SERVICE))) {
                                                // TODO Inhomogenous with node services

                                            this.logDebug("Invoking service [" + details.params.service + "] on Sensor [" +
                                                details.params.sensor + "] on Device [" + this.unmangleId(details.params.device) + "].");
                                            this.node[this.mangleId(details.params.node, details.params.device)][details.params.sensor][details.params.service](request.body);
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_SENSOR_STATE))) {
                                            // TODO Inhomogenous with node services

                                            this.logDebug("Receiving state [" + details.params.state
                                                + "] from Sensor [" + details.params.sensor
                                                + "] on Device [" + this.unmangleId(details.params.device) + "].");

                                            var mangledId = this.mangleId(details.params.node, details.params.device);
                                            var value = this.node[mangledId][details.params.sensor].state[details.params.state];
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_STORYBOARD))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_DATA, GET))) {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_DATA, PUT))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_PUSHALLSTATES, POST))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_DEVICES, POST))) {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_DEVICE, PUT))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_DEVICE, DELETE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_ACTORS, POST))) {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_ACTOR, PUT))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_ACTOR, DELETE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_SENSORS, POST))) {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_SENSOR, PUT))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_SENSOR, DELETE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_SERVICES, POST))) {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_SERVICE, PUT))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_SERVICE, DELETE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_GROUPS , POST))) {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_GROUP, PUT))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_GROUPS, DELETE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_EPS, POST))) {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_EP, PUT))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_EP, DELETE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_STORYBOARDS, POST))) {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_STORYBOARD, PUT))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_STORYBOARD, DELETE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_JOBS, POST))) {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_JOB, PUT))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_CONFIG_JOB, DELETE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_FILES, POST))) {
                                            if (!fs
                                                    .existsSync(this.options.dataDirectory
                                                        + "/files")) {
                                                fs
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_LOGS, DELETE))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_LOGS, POST))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_RESOURCE_STATE, GET))) {
                                        ...
                                        else if ((details = this.matchRequestPath(request, REQ_NODE_UPDATE_ENTITLEMENTS, PUT))) {

                                            var device =  this.node.findDevice(this.mangleId(request.nodeUuid, details.params.device));

                                            if (!device) {
                                                var msg = 'Cannot find device ' + details.params.device;
                                                this.logError(msg);
                                                this.emitError(request, msg);
                                            }

                                            var change = request.body;

                                            if (!change) {
                                                var msg = 'No changes passed for update entitlements call.';
                                                this.logError(msg);
                                                this.emitError(request, msg);
                                            }

                                            if (device.updateEntitlements) {
                                                device.updateEntitlements(change);
                                            } else {
                                                var msg = 'Device ' + device + ' does not implement AccessManager interface.';
                                                this.logError(msg);
                                                this.emitError(request, msg);
                                            }
                                        }
                                        else {
                                            var msg = 'Unsupported request path ' + request.path;
                                            this.logError(msg);
                                            this.emitError(request, msg);
                                        }
                                    }
     ...
     *
     */
    NodeManager.prototype.matchRequestPath = function (request, pattern, method) {
        ...
        return details && request.method === method ? details : null;
    };
    ...
    NodeManager.prototype.configure = function (node) {
        ...
        // required for configure in simulation mode
        if (this.__nodeManager == null) {
            this.node.__nodeManager = this;
        }
...
function Node() {
    ...
    Node.prototype.publishActorEvent = function (device, actor, event, data, relatedId) {
        this.logDebug("Publish Actor Event", device.label, actor.label, event, data);

        if (this.namespace) {
            this.namespace.emit("actorEvent", {
                node: this.uuid,
                device: device.id,
                actor: actor.id,
                timestamp: moment().toISOString(),
        ...
        if (this.reverseProxyNamespace) {
            ...
            this.reverseProxyNamespace.emit("actorEvent", {
                customer: configuration.customer,
                mesh: configuration.mesh,
                node: nodeUuid,
                gatewayConfiguration: configuration.id,
     ...
     *
     */
    Node.prototype.publishSensorStateChange = function (device, sensor, state) {
        try {
            if (this.namespace) {
                this.namespace.emit("sensorStateChange", {
                    node: this.uuid,
                    device: device.id,
                    sensor: sensor.id,
                    timestamp: moment().toISOString(),
            ...
            if (this.reverseProxyNamespace) {
                ...
                this.reverseProxyNamespace.emit("sensorStateChange", {
                    customer: configuration.customer,
                    mesh: configuration.mesh,
                    node: this.__nodeManager.extractNodeUuid(device.id),
                    gatewayConfiguration: configuration.id,
    ...
    Node.prototype.publishSensorEvent = function (device, sensor, event, data, relatedId) {
        this.logDebug("Publish Sensor Event", device.label, sensor.label, event, data);

        if (this.namespace) {
            this.namespace.emit("sensorEvent", {
                node: this.uuid,
                device: device.id,
                sensor: sensor.id,
                timestamp: moment().toISOString(),
        ...
        if (this.reverseProxyNamespace) {
            ...
            this.reverseProxyNamespace.emit("sensorEvent", {
                customer: configuration.customer,
                mesh: configuration.mesh,
                node: nodeUuid,
                gatewayConfiguration: configuration.id,
     ...
     *
     */
    Node.prototype.publishSensorStateChangeHistory = function (device, sensor, history) {
        try {
            if (this.namespace) {
                this.namespace.emit("sensorStateChangeHistory", {
                    node: this.uuid,
                    device: device.id,
                    sensor: sensor.id,
                    history: history
            ...
            if (this.reverseProxyNamespace) {
                ...
                this.reverseProxyNamespace.emit("sensorStateChangeHistory", {
                    customer: configuration.customer,
                    mesh: configuration.mesh,
                    node: this.__nodeManager.extractNodeUuid(device.id),
                    gatewayConfiguration: configuration.id,
    ...
    Node.prototype.publishFlowStart = function (service, parameters) {
        ...
        if (this.reverseProxyNamespace) {
            ...
            this.reverseProxyNamespace.emit("flowStart", message = {
                customer: configuration.customer,
                mesh: configuration.mesh,
                node: nodeUuid,
                gatewayConfiguration: configuration.id,
            ...
            this.logDebug('Send Flow Start Message', message);
        }
    };
