module.exports = {
    ...
    create: function (node, app, io, namespacePrefix) {
        var nodeManager = new NodeManager();

        nodeManager.createNode(node, app, io, namespacePrefix);
    ...
    bootstrap: function (options, app, io, server) {
        ...
        nodeManager.bootstrapNode(options, app, io, server);
    }
};
...
function NodeManager() {
     ...
     *
     */
    NodeManager.prototype.createNode = function (node, app, io, namespacePrefix) {
        this.loadPlugins();

        this.node = node;

        utils.inheritMethods(this.node, new Node());
        this.node.initialize(this.devicePlugins, true, "configured");

        this.node.lastConfigurationTimestamp = new Date()
            .getTime();

        this.initializeCommunicationBasis({simulated: true}, app, io, namespacePrefix);
     ...
     *
     */
    NodeManager.prototype.bootstrapNode = function (options, app, io, server) {
        io.listen(server);

        this.loadPlugins();

        if (options.proxy == "local") {
            this.loadNodeConfiguration(options.nodeConfigurationFile);
            utils.inheritMethods(this.node, new Node());
            this.node.initialize(this.devicePlugins, options.simulated, "configured");
            this.node.start();
            ...
            if (this.existsNodeConfiguration(options.nodeConfigurationFile)) {
                ...
                console
                    .log("\nNode to be started in proxy mode against server " + options.proxy + " with previously pushed configuration.");
                ...
                this.node.initialize(this.devicePlugins, options.simulated, "unconfigured");
                this.initializeCommunication(options, app, io, "");
    ...
    NodeManager.prototype.loadNodeConfiguration = function (nodeConfigurationFile) {
        try {
            if (nodeConfigurationFile.indexOf(".js", nodeConfigurationFile.length
                ...
                this.node = require(nodeConfigurationFile);
                this.configuration = JSON.parse(JSON.stringify(this.node))
            } else {
                this.node = JSON.parse(fs.readFileSync(nodeConfigurationFile, {
                    encoding: "utf-8"
                }));
                this.configuration = JSON.parse(JSON.stringify(this.node));
            }
     ...
     *
     */
    NodeManager.prototype.initializeCommunication = function (options, app, io, namespacePrefix) {
        this.initializeCommunicationBasis(options, app, io, namespacePrefix)
     ...
     *
     */
    NodeManager.prototype.initializeCommunicationBasis = function (options, app, io, namespacePrefix) {
        ...
        this.connectInterval = setInterval(function () {
            ...
            this.reverseProxyNamespace.on("httpRequest", function (request) {
                    ...
                    try {
                        ...
                        if (details = this.matchRequestPath(request.path, "/state")) {
    ...
    NodeManager.prototype.configure = function (node) {
        ...
        utils.inheritMethods(this.node,
            new Node());
        this.node.initialize(this.devicePlugins, this.options.simulated, "configured");
...
function Node() {
     ...
     *
     */
    Node.prototype.initialize = function (plugins, simulated, state) {
