...
function loadPlugins() {
	...
	for ( var n in list) {
		...
		if (dirStat && dirStat.isDirectory()
			...
			var pluginFiles = fs.readdirSync(__dirname + "/default-devices/"
					+ list[n]);

			for ( var m in pluginFiles) {
				var fileStat = fs.statSync(__dirname + "/default-devices/"
						+ list[n] + "/" + pluginFiles[m]);
				...
				if (fileStat && fileStat.isFile() && pluginFiles[m]) {
					...
					try {
						plugins[pluginId] = require(
								__dirname + "/default-devices/" + list[n] + "/"
										+ pluginId).create();
					} catch (x) {
						console
			...
			if (this.services[n].content.type == "storyboard") {
				...
				try {
					this[this.services[n].id] = new Function('input', this
							.preprocessScript(this.services[n].content.script));
