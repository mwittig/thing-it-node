module.exports = {
    ...
    bootstrapFromNode: function (node, options, app, io) {
        var nodeManager = new NodeManager();

        return nodeManager.bootstrapNode(node, options, app, io);
    }
};
...
function NodeManager() {
     ...
     *
     */
    NodeManager.prototype.initialize = function (node, options) {
     ...
     * @returns
     */
    NodeManager.prototype.run = function (app, io) {
        this.node.start();
        ...
        this.initializeCommunication(this.options, app, io, "");
    ...
    NodeManager.prototype.initializeRestServices = function () {
        ...
        this.app
            .post(
                ...
                function (req, res) {
                    try {
                        this.pair(req.body.mesh.label, req.headers['ti-sid']).then(function (gateway) {
                            res.send(gateway);
                        }.bind(this)).fail(function (error) {
                            res.status(500).send(error);
                        }.bind(this));
     ...
     * @returns {*|promise}
     */
    NodeManager.prototype.pair = function (meshLabel, sid, account, password) {
        ...
        getmac.getMac(function (error, macAddress) {
            if (error) {
                ...
                if (meshLabel) {
                    ...
                    for (var n in this.configurationsMap) {
                        this.configurationsMap[n].configuration.uuid = n;

                        mesh.nodes.push(this.configurationsMap[n].configuration);
                ...
                request.post({
                    ...
                    if (error) {
                        ...
                        if (response.statusCode == "401") {
                            ...
                            this.options.proxy = "https://www.thing-it.com";
                            ...
                            // TODO Do this cleanup somewhere else?
                            ...
                            for (var n in body.mesh.nodes) {
                                ...
                                delete body.mesh.nodes[n].nodeComputerUuid;
                            }

                            for (var n in body.mesh.nodes) {
                                var filePath;

                                if (this.configurationsMap[body.mesh.nodes[n].uuid]) {
                                    filePath = this.configurationsMap[body.mesh.nodes[n].uuid].filePath;
                                } else {
                                    filePath = process.cwd() + "/configurations/configuration-" + n + ".js";
                                }

                                this.logDebug("Updating Gateway Configuration [" + body.mesh.nodes[n].uuid + '] in File [' + filePath + "]");

                                fs.writeFileSync(filePath, "module.exports = " + JSON.stringify(body.mesh.nodes[n]) + ";", {
                                    encoding: "utf-8"
                                });
                            }
    ...
    NodeManager.prototype.updateFirmwareAndShutdown = function () {
        ...
        var child = exec('npm update thing-it-node -g',
                                ...
                                this.reverseProxyNamespace.on("httpRequest", function (request) {
                                    ...
                                    try {
                                        ...
                                        if ((details = this.matchRequestPath(request.path, "/firmware")) && request.method === "POST") {
                                            ...
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0, data: this.getState(request.nodeUuid)
                                            });
                                        }
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configurationUnmangled")) && request.method === "GET") {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0, data: {
                                                    state: this.state,
                                                    ...
                                                    configuration: this.unmangleNodeConfiguration(_.cloneDeep(this.cleanConfigurationCopy(request.nodeUuid)))
                                                }
                                            });
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configuration/devices")) && request.method === "POST") {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configuration/devices/:device")) && request.method === "PUT") {
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configuration/devices/:device")) && request.method === "DELETE") {
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configuration/devices/:device/actors")) && request.method === "POST") {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configuration/devices/:device/actors/:actor")) && request.method === "PUT") {
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configuration/devices/:device/actors/:actor")) && request.method === "DELETE") {
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configuration/devices/:device/sensors")) && request.method === "POST") {
                                            if (!request.body.id) {
                                                this.reverseProxyNamespace.emit("httpError", {
                                                    uuid: request.uuid,
                                                    status: 500,
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configuration/devices/:device/sensors/:sensor")) && request.method === "PUT") {
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/configuration/devices/:device/sensors/:sensor")) && request.method === "DELETE") {
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/devices/:device/state/:state/resource")) && request.method === "GET") {
        ...
        try {
            ...
            if (this.reverseProxyNamespace) {
                this.reverseProxyNamespace.emit("deviceRegistration", {
                    node: this.__nodeManager.defaultNodeConfiguration.uuid, // TODO?
                    device: device.id
                });
            }
        }
