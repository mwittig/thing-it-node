...
function Node() {
    ...
    Node.prototype.initializeReverseProxy = function () {
        ...
        console.log("Opening socket for reverse proxy " + reverseProxyNamespace);

        //var connectInterval = setInterval(function () {
        //    console.log("(Re)try initial NS connection");
        //
        var socket = new io.connect(reverseProxyNamespace, {
            reconnection: true,
        ...
        socket.on("http-proxy-request", function (request) {
            ...
            var proxyRequest = http.request(options, function (response) {
                ...
                response.on('end', function (chunk) {
                    socket.emit("http-proxy-response", response);
                });
                response.on('error', function (error) {
                    socket.emit("http-proxy-response", {status: 500});
                });
            }).on("error", function (e) {
                                        ...
                                        function () {
                                            ...
                                            if (!self[req.params.service]) {
                                                ...
                                                if (!req.body || !req.body.parameters) {
                                                    console.log("Service without body");
                                                    self[req.params.service]
                                                    ();
                                                }
                                                else {
                                                    console.log("Service with body");
                                                    self[req.params.service]
                                                    (req.body.parameters);
                                                }
