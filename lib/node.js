...
function NodeManager() {
    ...
    NodeManager.prototype.initializeRestServices = function () {
        ...
        this.app.post("/configuration/services", function (request, response) {
            ...
            this.logDebug("Service", request.body);
        ...
        this.app.put("/configuration/groups", function (request, response) {
            console.log("Update Groups", request.body);

            this.node.updateGroups(request.params.group, request.body).then(function () {
                response.send(this.node.clientCopy());
            }.bind(this)).fail(function (error) {
                this.logError(error);
                response.status(500).send(error);
                            ...
                            this.reverseProxyNamespace.on("connect", function () {
                                ...
                                this.reverseProxyNamespace.on("httpRequest", function (request) {
                                    ...
                                    try {
                                        ...
                                        else if (details = this.matchRequestPath(request.path, "/configuration/groups") && request.method === "PUT") {
                                            this.node.updateGroups(details.params.group, request.body).then(function () {
                                                this.reverseProxyNamespace.emit("httpResponse", {
                                                    uuid: request.uuid,
                                                    status: 0,
                                                    data: this.node.clientCopy()
...
function Node() {
    ...
    Node.prototype.clientCopy = function () {
        ...
        for (var n in this.services) {
            if (this.services[n].type === "storyboard") {
                copy.services.push({
                    ...
                    type: this.services[n].type,
                    content: this.services[n].content
                });
            }
            else {
                copy.services.push({
        ...
        for (var n in this.eventProcessors) {
            copy.eventProcessors.push({
                id: this.eventProcessors[n].id,
                label: this.eventProcessors[n].label,
                observables: this.eventProcessors[n].observables,
                trigger: this.eventProcessors[n].trigger,
                action: this.eventProcessors[n].action.clientCopy()
            });
        }
        ...
        for (var n in this.jobs) {
            copy.jobs.push({
                id: this.jobs[n].id,
                label: this.jobs[n].label,
                startTimestamp: this.jobs[n].startTimestamp,
                fromNextFullOf: this.jobs[n].fromNextFullOf,
                recurrence: this.jobs[n].recurrence,
                end: this.jobs[n].end,
                action: this.jobs[n].action.clientCopy()
            });
        }
     ...
     * @returns {*|promise}
     */
    Node.prototype.updateGroups = function (id, newGroups) {
        var deferred = q.defer();
