...
function NodeManager() {
    ...
    NodeManager.prototype.loadNodeConfigurations = function () {
        ...
        for (var n in list) {
            ...
            if (fileStat && fileStat.isFile()) {
                ...
                this.logDebug(nodeConfiguration);
                ...
                this.configurationsMap[nodeConfiguration.uuid] = {
                    configuration: JSON.parse(JSON.stringify(nodeConfiguration)), //  Keep clean copy of name-mangled configuration
    ...
    NodeManager.prototype.nameMangleNodeConfiguration = function (nodeConfiguration) {
        ...
        for (var n in nodeConfiguration.services) {
            ...
            if (nodeConfiguration.services[n].type === "script" && nodeConfiguration.services[n].content &&
                ...
                this.logDebug(nodeConfiguration.services[n].content.script);
            }
    ...
    NodeManager.prototype.extractNodeUuid = function (id) {
        console.log("Extract from " + id);

        id = id.substring(id.indexOf("ti__") + 4);

        console.log(id.substring(0, id.indexOf("__")).uuid.replace(/_/g, "-"));

        return id.substring(0, id.indexOf("__")).uuid.replace(/_/g, "-");
    };
    ...
    NodeManager.prototype.clientCopy = function (configuration) {
        ...
        // TODO All copy operations below should be in the respective classes
        ...
        for (var n in configuration.groups) {
            copy.groups.push(configuration.clientCopyGroup(configuration.groups[n]));
        }

        for (var n in configuration.services) {
            if (configuration.services[n].type === "storyboard") {
                copy.services.push({
                    id: configuration.services[n].id, label: configuration.services[n].label,
                    icon: configuration.services[n].icon,
                    photoUri: configuration.services[n].photoUri,
                    type: configuration.services[n].type,
                    content: configuration.services[n].content
                });
            }
            else {
                copy.services.push({
                    ...
                    photoUri: configuration.services[n].photoUri,
                    type: configuration.services[n].type,
                    parameters: configuration.services[n].parameters,
                    content: configuration.services[n].content
                });
            }
        }
        ...
        for (var n in configuration.eventProcessors) {
            copy.eventProcessors.push({
                id: configuration.eventProcessors[n].id,
                label: configuration.eventProcessors[n].label,
                observables: configuration.eventProcessors[n].observables,
                trigger: configuration.eventProcessors[n].trigger,
                action: configuration.eventProcessors[n].action.clientCopy()
            });
        }
    ...
    NodeManager.prototype.saveNodeConfiguration = function (uuid) {
        ...
        this.logDebug("Client Copy created for " + uuid);
        ...
        for (var n in clientCopy.devices) {
            if (this.belongsToConfiguration(clientCopy.devices[n].id, uuid)) {
                clientCopy.devices[n].id = this.unmangleId(clientCopy.devices[n].id);
        ...
        for (var n in clientCopy.services) {
            if (this.belongsToConfiguration(clientCopy.services[n].id, uuid)) {
                clientCopy.services[n].id = this.unmangleId(clientCopy.services[n].id);
        ...
        for (var n in clientCopy.eventProcessors) {
            if (this.belongsToConfiguration(clientCopy.eventProcessors[n].id, uuid)) {
                clientCopy.eventProcessors[n].id = this.unmangleId(clientCopy.eventProcessors[n].id);

                nodeConfiguration.eventProcessors.push(clientCopy.eventProcessors[n]);
                this.unmangleAction(clientCopy.eventProcessors[n].action);

                for (var m in clientCopy.eventProcessors[n].observables) {
                    clientCopy.eventProcessors[n].observables[m] = this.unmangleId(clientCopy.eventProcessors[n].observables[m]);
                }
            }
        }
        ...
        for (var n in clientCopy.jobs) {
            if (this.belongsToConfiguration(clientCopy.jobs[n].id, uuid)) {
                clientCopy.jobs[n].id = this.unmangleId(clientCopy.jobs[n].id);

                nodeConfiguration.jobs.push(clientCopy.jobs[n]);
                this.unmangleAction(clientCopy.jobs[n].action);
            }
        }
        ...
        for (var n in clientCopy.groups) {
            if (this.belongsToConfiguration(clientCopy.groups[n].id, uuid)) {
                clientCopy.groups[n].id = this.unmangleId(clientCopy.groups[n].id);

                nodeConfiguration.groups.push(clientCopy.groups[n]);
                this.unmangleGroup(clientCopy.groups[n])
            }
        }
        ...
        // Cache the unmangled Node Configuration

        this.configurationsMap[uuid].configuration = nodeConfiguration;
    };
     ...
     * @returns {*}
     */
    NodeManager.prototype.getCleanNodeConfiguration = function (uuid) {
        return this.configurationsMap[uuid].configuration;
    };
    ...
    NodeManager.prototype.initializeRestServices = function () {
        ...
        this.app.put("/configuration/groups/:group", function (request, response) {
            this.node.updateGroup(request.params.group, request.body).then(function (group) {
                response.send(group);
            }.bind(this)).fail(function (error) {
                this.logError(error);
                response.status(500).send(error);
    ...
    NodeManager.prototype.initializeReverseProxy = function () {
        ...
        var reverseProxyNamespaceUrl = this.options.proxy + "/proxy/nodeBoxes/" + this.uuid;
                                    ...
                                    try {
                                        ...
                                        if (details = this.matchRequestPath(request.path, "/plugins") && request.method === "GET") {
                                            ...
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                ...
                                                status: 0, data: {
                                                    state: this.state,
                                                    nodeBoxUuid: nodeManager.uuid,
                                                    configuration: nodeManager.configurationCopy(request.nodeUuid)
                                                }
                                            });
                                        }
...
function Node() {
    ...
    Node.prototype.clientCopy = function () {
        return nodeManager.clientCopy(this);
    };
    ...
    Node.prototype.publishStoryboardProgress = function (storyboard, elapsedTime) {
        this.logDebug("Publishing Storyboard progress on [" + nodeManager.unmangleId(storyboard.id) + "] at " + elapsedTime);
    ...
    Node.prototype.createDevice = function (newDevice) {
        ...
        newDevice.id = utils.getNextDefaultId(this.devices, nodeManager.mangleId(nodeManager.defaultNodeConfiguration.uuid, "device"), 1);

        this.devices.push(newDevice);
        device.bind(this, newDevice);

        newDevice.startDevice().then(function () {
            this.publishNodeConfigurationChange(nodeManager.defaultNodeConfiguration.uuid);
            deferred.resolve(newDevice.clientCopy());
    ...
    Node.prototype.updateDevice = function (id, newDevice) {
        ...
        oldDevice.stopDevice().then(function () {
            ...
            newDevice.startDevice().then(function () {
                this.publishNodeConfigurationChange(nodeManager.extractNodeUuid(id));
                deferred.resolve();
            }.bind(this)).fail(function () {
                utils.removeItemFromArray(this.devices, newDevice);
    ...
    Node.prototype.createActor = function (deviceId, newActor) {
        ...
        newActor.startActor().then(function () {
            this.publishNodeConfigurationChange(nodeManager.extractNodeUuid(deviceId));
            deferred.resolve();
        }.bind(this)).fail(function () {
    ...
    Node.prototype.createService = function (newService) {
        ...
        newService.id = utils.getNextDefaultId(this.services, nodeManager.mangleId(nodeManager.defaultNodeConfiguration.uuid, "service"), 1);

        this.services.push(newService);
        service.bind(this, newService);
        this.publishNodeConfigurationChange(nodeManager.defaultNodeConfiguration.uuid);

        deferred.resolve(newService.clientCopy());
    ...
    Node.prototype.createGroup = function (newGroup) {
        ...
        newGroup.id = utils.getNextDefaultId(this.groups, nodeManager.mangleId(nodeManager.defaultNodeConfiguration.uuid, "group"), 1);

        this.logDebug(">>> Group", newGroup);

        this.groups.push(newGroup);
        group.bind(this, newGroup);
        this.publishNodeConfigurationChange(nodeManager.defaultNodeConfiguration.uuid);
        deferred.resolve(newGroup.clientCopy());
    ...
    Node.prototype.createEventProcessor = function (newEventProcessor) {
        ...
        newEventProcessor.id = utils.getNextDefaultId(this.eventProcessors, nodeManager.mangleId(nodeManager.defaultNodeConfiguration.uuid, "eventProcessor"), 1);

        this.logDebug("Event Processor >>>", newEventProcessor);

        this.eventProcessors.push(newEventProcessor);
        eventProcessor.bind(this, newEventProcessor);
        this.publishNodeConfigurationChange(nodeManager.defaultNodeConfiguration.uuid);
        deferred.resolve(newEventProcessor.clientCopy());
    ...
    Node.prototype.createStoryboard = function (newStoryboard) {
        ...
        newStoryboard.id = utils.getNextDefaultId(this.services, nodeManager.mangleId(nodeManager.defaultNodeConfiguration.uuid, "storyboard"), 1);
    ...
    Node.prototype.createJob = function (newJob) {
        ...
        newJob.id = utils.getNextDefaultId(this.jobs, nodeManager.mangleId(nodeManager.defaultNodeConfiguration.uuid, "job"), 1);

        this.jobs.push(newJob);
        job.bind(this, newJob);
        this.publishNodeConfigurationChange(nodeManager.defaultNodeConfiguration.uuid);
        deferred.resolve(newJob.clientCopy());
    ...
    Node.prototype.updateJob = function (id, newJob) {
        ...
        this.jobs.push(newJob.clientCopy());
        job.bind(this, newJob);
        this.publishNodeConfigurationChange(nodeManager.extractNodeUuid(id));
        deferred.resolve(newJob.clientCopy());
    ...
    Node.prototype.publishNodeConfigurationChange = function (nodeConfigurationUuid) {
        try {
            nodeManager.saveNodeConfiguration(nodeConfigurationUuid);
        }
        catch (error) {
