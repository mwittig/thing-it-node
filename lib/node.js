...
function Node() {
    ...
    Node.prototype.initialize = function (options, app, io, server) {
        ...
        app.post("/login", function (req, res) {
            if (this.options.authentication.mode === "none") {
                res.send({});
            }
            else if (this.options.authentication.mode === "password") {
            ...
            } else if (this.options.authentication.mode === "user" && (!this.options.proxy || this.options.proxy.mode === "local")) {
                // Replace by login
    ...
    Node.prototype.initializeReverseProxy = function () {
        if (!this.options.proxy || !this.options.proxy.mode === "remote") {
            return;
        }

        var reverseProxyNamespaceUrl = this.options.proxy.reverseProxy + "/reverse-proxy/nodes/" + this.uuid;
