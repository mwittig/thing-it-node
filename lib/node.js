module.exports = {
    ...
    bootstrapFromNode: function (node, options, app, io, namespace) {
        var nodeManager = new NodeManager();

        return nodeManager.bootstrapNode(node, options, app, io, namespace);
    },
    bind: function (node) {
...
function NodeManager() {
     ...
     *
     */
    NodeManager.prototype.bootstrapNode = function (node, options, app, io, namespace) {
    ...
    NodeManager.prototype.initializeRestServices = function () {
        ...
        this.app.post("/data/:data", function (req, res) {
            this.logDebug("Creating data [" + req.params.data + "].");
                            ...
                            this.reverseProxyNamespace.on("connect", function () {
                                ...
                                this.reverseProxyNamespace.on("httpRequest", function (request) {
                                    ...
                                    try {
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/data/:data")) && request.method === "GET") {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0,
                                                data: this.node[details.params.data]
                                            });
                                        }
                                        else if ((details = this.matchRequestPath(request.path, "/data/:data")) && request.method === "POST") {
                                            if (request.body['ti-primitive-wrapper'])
                                            {
                                                this.node[request.params.data] = request.body['ti-primitive-wrapper'];
                                            } else {
                                                this.node[request.params.data] = request.body;
                                            }

                                            this.node.findData(details.params.data).update(request.body).then(function () {
                                                this.reverseProxyNamespace.emit("httpResponse", {
                                                    uuid: request.uuid,
                                                    status: 0
                                                });
...
function Node() {
    ...
    Node.prototype.publishDeviceEvent = function (device, event, data) {
