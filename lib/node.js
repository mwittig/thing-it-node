                            ...
                            this.reverseProxyNamespace.on("connect", function () {
                                ...
                                this.reverseProxyNamespace.on("httpRequest", function (request) {
                                    ...
                                    try {
                                        ...
                                        if ((details = this.matchRequestPath(request.path, "/nodes/:node/firmware"))
                                            && request.method === "POST") {
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/nodes/:node/plugins"))
                                            && request.method === "GET") {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                                status: 0,
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/nodes/:node/state"))
                                            && request.method === "GET") {
                                            this.reverseProxyNamespace.emit("httpResponse", {
                                                uuid: request.uuid,
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/nodes/:node/devices/:device/sensors/:sensor/services/:service"))) {
                                            // TODO Inhomogenous with node services
                                        ...
                                        else if ((details = this.matchRequestPath(request.path, "/nodes/:node/devices/:device/sensors/:sensor/state/:state"))) {
                                            // TODO Inhomogenous with node services

                                            this.logDebug("Receiving state [" + details.params.state + "] from Sensor [" +
                                                details.params.sensor + "] on Device [" + this.unmangleId(details.params.device) + "].");

                                            var value = this.node[this.mangleId(details.params.node, details.params.device)][details.params.sensor].state[details.params.state];
