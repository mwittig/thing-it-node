...
function Node() {
    ...
    Node.prototype.initialize = function (options, app, io, server) {
        ...
        app.get("/authentication", function (req, res) {
            res.send(self.options.authentication);
        });
        app.post("/login", function (req, res) {
            if (this.options.authentication.type == "none") {
                res.send({});
            }
            else if (this.options.authentication.type == "password") {
            ...
            } else if (this.options.authentication.type == "local") {
                // Replace by login
                ...
                this.getUser(req.body.account).then(function (user) {
                    ...
                    if (req.body.account == "root") {
                        var user = {
                            ...
                            firstName: "Root",
                            lastName: "User",
                            entitlements: [{type: "role", id: "administrator", label: "Administrator", granted: true}]
                        };
    ...
    Node.prototype.initializeReverseProxy = function () {
        ...
        var reverseProxyNamespaceUrl = this.options.reverseProxy + "/reverse-proxy/nodes/" + this.uuid;
        var connectInterval = setInterval(function () {
            ...
            this.reverseProxyNamespace.on("request", function (request) {
                console.debug("Request " + request.path);
                ...
                try {
                    ...
                    var proxyRequest = http.request(options, function (response) {
                        response.on('data', function (data) {
                            ...
                            this.reverseProxyNamespace.emit("response-data", {status: 0, data: data});
                        }.bind(this));
                        response.on('end', function () {
                            console.debug("Response End");

                            this.reverseProxyNamespace.emit("response-end", {status: 0});
                        }.bind(this));
                        response.on('error', function (error) {
                            console.debug("Response Error: " + error);

                            this.reverseProxyNamespace.emit("response-error", {status: 500, error: error});
                        }.bind(this));
    ...
    Node.prototype.start = function (app, io) {
        ...
        utils
            ...
            function () {
                ...
                utils
                    .promiseSequence(self.devices, 0, "startDevice")
                    .then(
                    function () {
                        try {
                            // Event Processors
    ...
    Node.prototype.createEntitlementsForUser = function (user) {
        ...
        for (var n in user.entitlements) {
            if (user.entitlements[n].type == "role") {
                newEntitlements.push(user.entitlements[n]);
            }
        }

        for (var n in this.groups) {
            console.log("Group " + this.groups[n].id);

            newEntitlements.push(this.createEntitlementForGroup(this.groups[n], true));
        }

        for (var n in newEntitlements) {
            for (var m in user.entitlements) {
                if (newEntitlements[n].type == "group" && user.entitlements[m].type == "group" && newEntitlements[n].id == user.entitlements[m].id) {
                    this.reconcileGroupEntitlements(newEntitlements[n], user.entitlements[m])
                }
            }
        }
     ...
     * @param group
     */
    Node.prototype.createEntitlementForGroup = function (group, topLevel) {
        var entitlement = {
            type: "group",
            id: group.id,
            label: group.label,
        ...
        if (group.devices) {
            for (var n in group.devices) {
                entitlement.devices.push(this.createEntitlementForDevice(this.findDevice(group.devices[n])));
            }
        }

        if (group.subGroups) {
            for (var n in group.subGroups) {
                entitlement.subGroups.push(this.createEntitlementForGroup(group.subGroups[n]));
            }
        }
     ...
     * @param actor
     */
    Node.prototype.createEntitlementForDevice = function (device) {
        var entitlement = {
            type: "device",
            label: device.label,
            path: device.id
     ...
     * @param actor
     */
    Node.prototype.createEntitlementForActor = function (actor) {
        var entitlement = {
            type: "actor",
