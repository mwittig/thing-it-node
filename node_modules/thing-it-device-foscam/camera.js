...
function Camera() {
    ...
    Camera.prototype.tiltUp = function (parameters) {
        ...
        if (this.isSimulated()) {
            ...
            if (this.lastTiltUp) {
                if (new Date().getTime() - this.lastTiltUp < 2000) {
                    this.tiltUpCount++;
                }
            } else {
                this.tiltUpCount = 0;
            ...
            this.lastTiltUp = new Date().getTime();

            this.logDebug("Tilt up Count", this.tiltUpCount);

            if (this.tiltUpCount > 5) {
     ...
     *
     */
    Camera.prototype.getResourceUrl = function (stateVariable) {
        if (this.isSimulated()) {
            return __dirname + "/data/spaceship.m4v"
        } else {
            return 'http://' + this.configuration.host + ':' + this.configuration.port + '/videostream.cgi?user=' + this.configuration.user +
