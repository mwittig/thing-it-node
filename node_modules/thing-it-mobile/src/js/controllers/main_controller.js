function MobileConsole($scope, $rootScope, $http) {
    ...
    this.consoleService.initializeSettings();
    this.refreshServer();
}
...
MobileConsole.prototype.refreshServer = function () {
    ...
    if (!this.isLocalMode()) {
        if (window.localStorage["thing-it.account"] != undefined && window.localStorage["thing-it.password"] != undefined &&
            ...
            this.login({
                account: window.localStorage["thing-it.account"],
                password: window.localStorage["thing-it.password"]
            });
        }
        else {
            this.rootPage(new LoginPage().initialize(this));
        }
    }
    else {
        ...
        this.consoleService
            ...
            .done(
            function (settings) {
                this.consoleService.settings.authentication = settings.authentication;

                if (this.consoleService.settings.authentication == "none") {
                    this.login();
                }
                else {
                    this.rootPage(new LoginPage().initialize(this));
                ...
                this.consoleService.logError("Cannot obtain Settings for local Node Box: " + error);
                this
 ...
 *
 */
MobileConsole.prototype.isLocalMode = function () {
    return this.consoleService.settings.server === "local";
}
...
MobileConsole.prototype.showPage = function (page, object) {
    ...
    promise.done(function () {
        ...
        this.consoleService.logError(error);
    }.bind(this));
};
                    ...
                    if (this.isLocalMode()) {
                        this.consoleService
                            ...
                            function (node) {
                                ...
                                this.consoleService.logDebug("Node 1 ==> ", node);
                                this.consoleService.logDebug("Logged-In User ===>", this.loggedInUser);
...
MobileConsole.prototype.connectNode = function (node) {
    ...
    this.namespace.on("message", function (message) {
        this.consoleService.logDebug("Receiving message");
        this.consoleService.logDebug(message);
    }.bind(this));
    this.namespace
        .on(
        "event",
        function (event) {
            this.consoleService.logDebug("Receiving event");
            this.consoleService.logDebug(event);

            if (event.sensor) {
                this.consoleService.logDebug(node.getDevice(
                ...
                this.consoleService.logDebug("Trigger event:");
                this.consoleService.logDebug(JSON.stringify(event));
    ...
    this.namespace.on("actorStateChange", function (actorStateChange) {
        this.consoleService.logDebug("Receiving Actor State Change");
        this.safeApply(function () {
            this.onActorStateChanged(actorStateChange);
        }.bind(this));
    }.bind(this));
    this.namespace.on("storyboardProgress", function (storyboardProgress) {
        this.consoleService.logDebug("Storyboard Progress ");
        this.consoleService.logDebug(storyboardProgress);
    ...
    this.namespace.on("deviceAdvertisement", function (device) {
        this.consoleService.logDebug("Device advertisement ");
        this.consoleService.logDebug(device);
 ...
 *
 */
MobileConsole.prototype.openNodePage = function (node) {
    this.consoleService.logDebug("Open Node Page");

    this.consoleService
        .getNode(node)
        .done(
        function (node) {
            this.consoleService.logDebug("Push Node Page");
 ...
 *
 */
MobileConsole.prototype.pushDeviceConfigurationDialog = function (device) {
    this.pushPage(new DeviceConfigurationDialog().initialize(this, device));
};
 ...
 *
 */
MobileConsole.prototype.pushActorConfigurationDialog = function (device, actor) {
    this.pushPage(new ActorConfigurationDialog().initialize(this, device, actor));
};
 ...
 *
 */
MobileConsole.prototype.pushSensorConfigurationDialog = function (device, sensor) {
    this.pushPage(new SensorConfigurationDialog().initialize(this, device, sensor));
};
 ...
 *
 */
MobileConsole.prototype.pushGroupConfigurationPage = function (group) {
    this.pushPage(new GroupConfigurationPage().initialize(this, group));
};

/**
 *
 */
MobileConsole.prototype.pushServiceConfigurationPage = function (service) {
    this.pushPage(new ServiceConfigurationPage().initialize(this, service));
};

/**
 *
 */
MobileConsole.prototype.pushNotificationsPage = function () {
    this.pushPage(new NotificationsPage().initialize(this));
};
...
MobileConsole.prototype.onDeviceStateChanged = function (stateChange) {
    try {
        ...
        this.consoleService.logDebug(error);

        // Node may not be bound and Device may not be registered yet and is already firing updates
    }
};
...
MobileConsole.prototype.onActorStateChanged = function (stateChange) {
    try {
        ...
        // Node may not be bound and Actor not be registered yet and is already firing updates
    }
};
